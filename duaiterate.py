...
def exec_DUAIterate(folders, via_trips):
    # duaiterate for iterative assigment
    # cfg_files = os.listdir(folders.O)
    print(f"5. Execute DUAITERATE")
    ...
    net_update = folders.dua_network_update  # network update (i.e., verify netwrok status) each 600s
    ...
    # change directory to duaiterate
    os.chdir(os.path.join(folders.SUMO_tool, 'duaiterate'))
    os.system(cmd)
    ...
    cmd = f'cp {summary_liter} {folders.SUMO_tool}/outputs/{curr_name}_summary.xml'
    os.system(cmd)
    cmd = f'cp {tripinfo_liter} {folders.SUMO_tool}/outputs/{curr_name}_tripinfo.xml'
    os.system(cmd)
    cmd = f'cp {fcd_liter} {folders.SUMO_tool}/outputs/{curr_name}_fcd.xml'
    os.system(cmd)
